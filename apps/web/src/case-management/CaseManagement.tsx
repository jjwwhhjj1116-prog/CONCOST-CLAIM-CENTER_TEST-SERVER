  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const result = createdCase ? { case: createdCase } : await apiRequest<{ case: CaseRecord }>('/api/cases', {
        method: 'POST', headers: { 'Idempotency-Key': idempotencyKey }, body: JSON.stringify({ title, claimType, description, clientName, clientLegalPosition, clientPositionDetail, category: { major: '건설 클레임', middle: claimType, minor: '사건 업무' } })
      });
  ...
  return <div className="case-create-page">
    ...
    <Card title="사건 기본정보" className="case-create-card">
      <form className="case-create-form" onSubmit={(event) => void submit(event)}>
        ...
        <Input label="클라이언트명" value={clientName} maxLength={300} required placeholder="제안서를 받을 법인·조합·발주처의 정확한 명칭" onChange={(event) => { setClientName(event.target.value); invalidateReview(); }} />
