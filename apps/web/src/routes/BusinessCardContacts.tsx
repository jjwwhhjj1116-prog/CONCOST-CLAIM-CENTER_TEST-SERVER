...
export function BusinessCardContacts({ mode, roles, onNavigate }: { mode:BusinessCardMode; roles:string[]; onNavigate:(path:string)=>void }): React.ReactElement {
  ...
  const analyze=async()=>{
    ...
    try{const body=new FormData();body.set('file',file);const result=await apiRequest<{analysis:Analysis}>('/api/business-cards/analyze',{method:'POST',body,timeoutMs:105_000});setAnalysis(result.analysis);setFields(result.analysis.fields);setNotice('Gemini 인식 완료 · 원본 명함과 아래 값을 대조한 뒤 등록해 주세요.');setAnalysisProgress({status:'complete'});}
  ...
  if(mode==='REGISTER')return <section className="route-view business-card-page" aria-labelledby="business-card-register-title">
    <AiGenerationProgressModal isOpen={Boolean(analysisProgress)} status={analysisProgress?.status??'running'} title="Gemini가 명함 정보를 읽고 있습니다" description="촬영본을 안전하게 전송하고 이름·회사·부서·직함·연락처를 구조화하고 있습니다." stages={['이미지 품질 확인','Gemini 글자·문맥 인식','연락처 필드 구조화','사람 검수용 결과 준비']} completeMessage="명함 인식이 완료되었습니다. 원본과 인식값을 대조하고 필요한 항목을 수정한 뒤 등록하세요." errorMessage={analysisProgress?.error} confirmLabel="인식값 확인·수정" timeoutHintSeconds={90} retryLabel="Gemini 인식 다시 시도" onRetry={()=>void analyze()} onConfirm={()=>setAnalysisProgress(null)} onClose={()=>setAnalysisProgress(null)}/>
