...
function proposalChapterWithCompanyImages(chapter:ProposalChapter,assets:readonly CompanyAsset[]):ProposalChapter{
  ...
  const lines=chapter.body.replaceAll('\r\n','\n').split('\n');let bodyChanged=false;let editorChanged=false;
  ...
  return bodyChanged||editorChanged?{...chapter,body:lines.join('\n'),editorJson:nextEditorJson}:chapter;
}
...
export const ProposalView:React.FC<ProposalViewProps>=({routeId,roles,userEmail='',onNavigate})=>{
  ...
  const download=async(format:FinalDocumentFormat)=>{if(!activeProposal||!selectedCaseId||!finalPreviewRef.current||!activeProposal.currentVersionId)return;setBusy(true);setErrorMessage(null);try{const result=await downloadFinalDocument({root:finalPreviewRef.current,format,fileName:`${cases.find((item)=>item.id===selectedCaseId)?.caseNumber??'CONCOST'}_${projectTitle}_v${activeProposal.version}`,orientation:'portrait',onProgress:(message)=>setSuccessMessage(message)});setSuccessMessage(`${format.toUpperCase()} 확정본 ${result.pageCount}페이지 내려받기 완료 · 담당자 검수와 동일한 A4 세로 화면을 페이지 단위로 보존했습니다.`);}catch(reason){setErrorMessage(reason instanceof Error?reason.message:'제안서를 내려받지 못했습니다.');}finally{setBusy(false);}};
  ...
  return <div className="proposal-view-container proposal-studio-v2" onInputCapture={(event)=>{const target=event.target as HTMLElement;if(target.closest('.proposal-stage'))setDirty(true);}} onChangeCapture={(event)=>{const target=event.target as HTMLElement;if(target.closest('.proposal-stage')&&target.matches('input, textarea, select, [contenteditable="true"]'))setDirty(true);}}>
    ...
    {activeProposal&&<Card title={`${activeProposal.title} · 제안서 스튜디오 · 작성 4단계 + 메일 준비`} className="proposal-step-card">
        ...
        <div className="proposal-editor-grid">
          ...
          <main className="proposal-chapter-editor">
            ...
            {reviewSurface==='chapter'&&<>
              ...
              <input ref={proposalImageInputRef} hidden type="file" accept=".jpg,.jpeg,.png,.webp,image/jpeg,image/png,image/webp" onChange={(event)=>void uploadProposalImage(event.target.files?.[0])}/>
      ...
      {step===4&&<section className="proposal-finalization-workspace">
        ...
        <aside className="proposal-finalization-actions" aria-label="제안서 확정 및 내보내기"><Button className="proposal-action-revise" onClick={()=>setStep(3)} disabled={busy}>← 수정 · 3단계로</Button>{activeProposal.status==='APPROVED'?<><Button className="final-export-button is-docx" aria-label="확정 제안서 Word DOCX 내려받기" title="미리보기와 동일한 DOCX 내려받기" onClick={()=>void download('docx')} disabled={busy}><FileFormatIcon format="docx"/><span>Word DOCX</span></Button><Button className="final-export-button is-pdf" aria-label="확정 제안서 PDF 내려받기" title="미리보기와 동일한 PDF 내려받기" onClick={()=>void download('pdf')} disabled={busy}><FileFormatIcon format="pdf"/><span>PDF</span></Button><Button className="final-export-button is-hwp" aria-label="확정 제안서 HWP 내려받기" title="편집기를 열지 않고 확정 HWP 내려받기" onClick={()=>void download('hwp')} disabled={busy}><FileFormatIcon format="hwp"/><span>HWP</span></Button><Button onClick={()=>onNavigate('/workflow/award?caseId='+encodeURIComponent(selectedCaseId)+'&proposalId='+encodeURIComponent(activeProposal.id))}>프로젝트 접수로 →</Button><Button variant="secondary" onClick={()=>goToProposalStep(5)} disabled={busy}>5단계 · 메일 준비</Button></>:<Button className="proposal-action-confirm" onClick={()=>setConfirmProposalOpen(true)} disabled={busy||!canEdit||activeProposal.status!=='DRAFT'}>확정 · 프로젝트 접수로</Button>}</aside>
        </div>
      </section>}
