...
const renderProposalBodyHtml=(body:string,assets:readonly CompanyAsset[],hydrateCompanyAssets:boolean):string=>{
  ...
  return DOMPurify.sanitize(typeof rendered==='string'?rendered:'',{
    ADD_ATTR:['data-image-align','data-table-width','data-table-align','data-table-density','data-cell-vertical-align','data-row-height-mm','colspan','rowspan','style','target','rel','width','height']
  });
};
function ProposalRichContent({body,editorJson,assets=[],hydrateCompanyAssets=true}:{body:string;editorJson?:import('@tiptap/core').JSONContent|null;assets?:CompanyAsset[];hydrateCompanyAssets?:boolean}):React.ReactElement{
  ...
  const html=structuredHtml
    ? DOMPurify.sanitize(deduplicateProposalImages(structuredHtml),{ADD_ATTR:['data-image-align','data-table-width','data-table-align','data-table-density','data-cell-vertical-align','data-row-height-mm','colspan','rowspan','style','target','rel','width','height']})
...
export const ProposalView:React.FC<ProposalViewProps>=({routeId,roles,userEmail='',onNavigate})=>{
  ...
  const applyVersion=useCallback((version:ProposalVersion|undefined,caseRow?:CaseItem)=>{
    ...
    try{
      ...
      if(Array.isArray(parsed.chapters)&&parsed.chapters.length===12){
        ...
        setClientName(linkedClientName||String(parsed.clientName??''));setProjectTitle(String(parsed.projectTitle??caseRow?.title??''));setSubtitle(String(parsed.subtitle??'건설 클레임 전문용역 제안'));setSubmissionDate(String(parsed.submissionDate??today()));
  ...
  const download=async(format:FinalDocumentFormat)=>{if(!activeProposal||!selectedCaseId||!finalPreviewRef.current||!activeProposal.currentVersionId)return;setBusy(true);setErrorMessage(null);try{if(format==='docx'||format==='pdf'){const result=await apiDownloadPost(`/api/cases/${encodeURIComponent(selectedCaseId)}/proposals/${encodeURIComponent(activeProposal.id)}/render`,{format,versionId:activeProposal.currentVersionId,version:activeProposal.version});triggerBrowserDownload(result);setSuccessMessage(`${format.toUpperCase()} 세로형 확정본 내려받기 완료 · Word에서 편집 가능한 본문과 표, 페이지 여백을 적용했습니다.`);return;}const result=await downloadFinalDocument({root:finalPreviewRef.current,format,fileName:`${cases.find((item)=>item.id===selectedCaseId)?.caseNumber??'CONCOST'}_${projectTitle}_v${activeProposal.version}`,orientation:'portrait',onProgress:(message)=>setSuccessMessage(message)});setSuccessMessage(`${format.toUpperCase()} 확정본 ${result.pageCount}페이지 내려받기 완료 · 화면 미리보기와 동일한 A4 세로 출력본입니다.`);}catch(reason){setErrorMessage(reason instanceof Error?reason.message:'제안서를 내려받지 못했습니다.');}finally{setBusy(false);}};
    ...
    {activeProposal&&<Card title={`${activeProposal.title} · 4단계 제안서 스튜디오`} className="proposal-step-card">
      ...
      {step===1&&<div className="proposal-stage proposal-stage-input">
        ...
        <div className="proposal-input-grid">
          ...
          <label className="proposal-field proposal-step1-textarea"><span>당 현장의 핵심 쟁점 분석 <i className="ui-required-mark">*</i></span><textarea required aria-required="true" value={keyIssues} onChange={(event)=>{setKeyIssues(event.target.value);setDirty(true);}} placeholder="계약조건, 물가변동 기준일, 단가조정 등 확인된 사실"/></label>
          ...
          <label className="proposal-field proposal-step1-textarea"><span>업무 수행 내용 <i className="ui-required-mark">*</i></span><textarea required aria-required="true" value={planNotes} onChange={(event)=>{setPlanNotes(event.target.value);setDirty(true);}}/></label>
      ...
      {step===3&&<div className="proposal-stage proposal-editor-stage">
        ...
        <div className="proposal-editor-grid">
          ...
          {reviewSurface==='chapter'?<aside className="proposal-module-panel"><h4>회사 공통 모듈 · 4~12장</h4><p>선택하면 저장된 최신 본문이 현재 장에 즉시 표시됩니다.</p>{modules.map((module)=><label key={module.code}><input type="checkbox" checked={includedModuleCodes.includes(module.code)} onChange={(event)=>setCompanyModuleIncluded(module,event.target.checked)}/><span><b>{module.chapterNumber}. {module.title}</b><small>중앙 승인본 v{module.version} · {includedModuleCodes.includes(module.code)?'적용 중':'제외'}</small></span></label>)}</aside>:<aside className="proposal-module-panel proposal-frontmatter-help"><h4>{reviewSurface==='cover'?'갑지 고정 양식':'목차 편집 기준'}</h4><p>{reviewSurface==='cover'?'하단 회사 로고·주소·연락처와 외곽선은 고정됩니다. 위 입력 항목만 프로젝트마다 바꾸세요.':'목차 제목을 바꾸면 해당 챕터 제목과 최종 합본에 동시에 반영됩니다.'}</p><strong>A4 세로형 고정</strong><small>갑지·목차·본문 모두 방향이 섞이지 않습니다.</small></aside>}
        </div>
        {canManageModules&&<>
