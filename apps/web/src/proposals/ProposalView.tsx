...
function ProposalFinalDocumentPreview({projectTitle,subtitle,clientName,submissionDate,chapters}:{projectTitle:string;subtitle:string;clientName:string;submissionDate:string;chapters:ProposalChapter[]}):React.ReactElement{
  return <article className="proposal-final-document" aria-label="확정 전 제안서 전체 합본 미리보기">
    <section className="proposal-final-cover" data-export-page><span>CONCOST CLAIM CENTER STUDIO</span><h3>{projectTitle}</h3><p>{subtitle}</p><dl><div><dt>제출처</dt><dd>{clientName}</dd></div><div><dt>제출일</dt><dd>{submissionDate}</dd></div><div><dt>제안사</dt><dd>주식회사 컨코스트 · 클레임센터</dd></div></dl><b>건설 클레임 전문용역 제안서</b></section>
    <section className="proposal-final-toc" data-export-page><span>TABLE OF CONTENTS</span><h3>목차</h3><ol>{chapters.map((item)=><li key={item.number}><b>{String(item.number).padStart(2,'0')}</b><span>{item.title}</span></li>)}</ol></section>
    {chapters.map((item)=><section className="proposal-final-chapter" data-export-page data-chapter-number={item.number} key={item.number}><header><span>CHAPTER {String(item.number).padStart(2,'0')}</span><h3>{item.number}. {item.title}</h3></header><ProposalRichContent body={item.body} editorJson={item.editorJson}/></section>)}
  </article>;
}
...
export const ProposalView:React.FC<ProposalViewProps>=({routeId,roles,userEmail='',onNavigate})=>{
  ...
  const loadProposalDetail=useCallback(async(cId:string,pId:string)=>{const response=await apiRequest<{proposal:Proposal}>(`/api/cases/${cId}/proposals/${pId}`);setActiveProposal(response.proposal);applyVersion(response.proposal.versions?.[0],cases.find((item)=>item.id===cId));setDirty(false);},[applyVersion,cases]);
  ...
  const saveVersion=async(generationMode:'MANUAL'|'AI',nextStep:3|4=4)=>{
    ...
    try{
      ...
      await loadCaseData(selectedCaseId);await loadProposalDetail(selectedCaseId,target.id);setDirty(false);if(generationMode==='AI')setAiGeneration({kind:'draft',status:'complete'});else setStep(nextStep);
  ...
  const confirmProposal=async()=>{if(!activeProposal||!selectedCaseId)return;setBusy(true);try{await apiRequest(`/api/cases/${selectedCaseId}/proposals/${activeProposal.id}/reviews`,{method:'POST',body:JSON.stringify({action:'CONFIRM',comment:'4단계 전체 합본 미리보기 확인 후 작성자가 최종 확정함',versionId:activeProposal.currentVersionId,version:activeProposal.version})});await loadProposalDetail(selectedCaseId,activeProposal.id);setConfirmProposalOpen(false);setSuccessMessage('제안서 확정 완료. 현재 미리보기 그대로 DOCX·PDF·HWP 내려받기가 활성화됐고 제안서 DB에 보관되었습니다.');}catch(reason){setErrorMessage(reason instanceof Error?reason.message:'제안서 상태를 바꾸지 못했습니다.');}finally{setBusy(false);}};
  ...
  return <div className="proposal-view-container proposal-studio-v2" onInputCapture={(event)=>{const target=event.target as HTMLElement;if(target.closest('.proposal-stage'))setDirty(true);}} onChangeCapture={(event)=>{const target=event.target as HTMLElement;if(target.closest('.proposal-stage')&&target.matches('input, textarea, select, [contenteditable="true"]'))setDirty(true);}}>
    ...
    {activeProposal&&<Card title={`${activeProposal.title} · 4단계 제안서 스튜디오`} className="proposal-step-card">
      ...
      {step===3&&<div className="proposal-stage proposal-editor-stage">
        <header className="workflow-stage-title"><div><b>STEP 3</b><h3>목차를 눌러 1~12장 전체를 직접 검수·수정하세요.</h3><p>모든 장에서 제목·글꼴·크기·색상·목록·표·원본 이미지·검색/치환을 사용할 수 있습니다. 4~12장은 중앙 기본값의 제안서별 복사본이므로 여기서 수정해도 회사 공통 DB 원본은 바뀌지 않습니다.</p></div>{stepThreeDocumentTools}</header>
        ...
        <div className="proposal-next"><Button variant="secondary" onClick={()=>setStep(2)}>← 초안 작성 방식</Button><Button className="proposal-action-confirm" onClick={()=>void saveVersion('MANUAL')} disabled={busy||!canEdit}>검수 완료 · 전체 합본 미리보기 →</Button></div>
      </div>}
      {step===4&&<section className="proposal-finalization-workspace">
        ...
        <div className={`proposal-finalization-status is-${activeProposal.status.toLowerCase()}`}><b>{activeProposal.status==='APPROVED'?'✓ 제안서 확정·DB 보관 완료':'확정 전 전체 내용 확인 중'}</b><span>{activeProposal.status==='APPROVED'?'아래 3종 내려받기 버튼이 활성화되었습니다. 확정본은 제안서 DB 이력에 보관됩니다.':'갑지·목차·12개 챕터·이미지를 마지막으로 확인한 뒤 확정하세요.'}</span></div>
