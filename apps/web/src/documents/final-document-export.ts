...
const canvasPage = async (source: HTMLCanvasElement, top: number, height: number): Promise<CapturedPage> => {
  const a4Canvas = document.createElement('canvas');
  a4Canvas.width = source.width;
  a4Canvas.height = Math.round(source.width * A4_RATIO);
...
const capturePages = async (root: HTMLElement, onProgress?: (message: string) => void): Promise<CapturedPage[]> => {
  ...
  for (let index = 0; index < elements.length; index += 1) {
    ...
    const pageHeight = Math.max(1, Math.floor(canvas.width * A4_RATIO));
    for (let top = 0; top < canvas.height; top += pageHeight) {
      result.push(await canvasPage(canvas, top, Math.min(pageHeight, canvas.height - top)));
    }
    canvas.width = 1;
    canvas.height = 1;
  }
...
const createHwp = async (pages: CapturedPage[], title: string, onProgress?: (message: string) => void): Promise<Uint8Array> => {
  ...
  try {
    const hwpx = createHwpx(pages, title);
...
export async function downloadFinalDocument(options: {
  ...
  const pages = await capturePages(options.root, options.onProgress);
